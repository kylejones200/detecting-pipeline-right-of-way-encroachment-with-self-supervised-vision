...
This is a Databricks + PySpark + DINOv2 implementation optimized for continental pipeline networks. The architecture processes daily high-resolution satellite or drone imagery, extracts embeddings on GPU clusters, performs distributed clustering with MLlib, and surfaces outliers ranked by anomaly score—no labels, no training data, no manual annotation.

![Pipeline ROW Encroachment Detection](15_row_encroachment_dinov2_main.png)

*Embedding space visualization (t-SNE projection) of 20,000 pipeline corridor tiles. Normal vegetation (green), maintained ROW (blue), and encroachment events (red clusters) separate naturally in self-supervised feature space. Distance to cluster centroid serves as anomaly score, with top 200 outliers (>3σ) flagged for 24-hour field inspection.*

## The Encroachment Problem: Scale vs Inspection Budget

### Regulatory Requirements

Federal pipeline safety regulations (49 CFR §195.412, §192.706) mandate aerial surveillance (monthly to annually depending on class location), ROW patrol (quarterly to annually for above-ground hazards), and encroachment detection (identification of "third-party activities" that could damage infrastructure).

For a 10,000-mile pipeline network, the costs are substantial. Monthly aerial patrol costs $500-2,000 per mile times 10,000 miles equals $5M-20M annually. Human analyst time is 10,000 miles divided by 20 miles per hour review equals 500 hours per month. Detection lag is 15-30 days (halfway between patrols).
...
The Bronze layer handles raw imagery, processing high-resolution satellite (Maxar, Planet, Airbus) or drone imagery at 0.3-1.0m resolution. Tile extraction creates 224×224 pixel patches along 500m ROW buffer. Metadata includes date, GPS coordinates, tile_id, and source.

The Silver layer creates embeddings through DINOv2-small (ViT-S/14) inference on GPU cluster, producing 384-dimensional embeddings per tile, stored as Delta tables with vector columns.

The Gold layer generates anomaly scores using K-means clustering (k=20-50 depending on corridor diversity), calculating distance to nearest centroid as anomaly score. Results are ranked by score, filtered by min distance threshold, and joined back to geospatial coordinates.

### Technology Stack

DINOv2, Meta's self-supervised vision transformer pre-trained on 142M images, requires no labels for deployment. It produces 384-dim embeddings (ViT-S/14) or 768-dim (ViT-B/14) with strong generalization to industrial and infrastructure imagery.

Databricks MLlib provides distributed machine learning for large-scale clustering. K-means handles millions of tiles, scales horizontally across workers, and integrates natively with Delta Lake.
...
Self-supervised learning eliminates labeling burden as DINOv2 pre-trained on 142M images generalizes to construction equipment, ground disturbance, and structures without pipeline-specific annotation. Embedding-based anomaly detection scales to millions of tiles when K-means clustering on 384-d embeddings processes 20,000 tiles in minutes on distributed Spark cluster. Outlier detection reduces review workload by 98.5% through inspecting 200 highest-anomaly tiles (1%) versus 20,000 total tiles, delivering 70-80% recall on actual encroachments. Databricks handles computer vision at pipeline scale with GPU clusters for inference, Delta Lake for vector storage, MLlib for distributed clustering, and Unity Catalog for governance. Cost-effectiveness versus traditional patrol shows daily satellite monitoring at $0.03/mile/day compared to monthly helicopter patrol at $50-200/mile enables 500x coverage increase. Early detection prevents incidents as Williams Transco encroachment was visible 6 weeks before rupture, while daily monitoring would have flagged within 24 hours.
...
Traditional pipeline monitoring relies on human analysts reviewing thousands of aerial images monthly, looking for changes they may or may not recognize. DINOv2 + Databricks transforms this into automated anomaly detection: self-supervised embeddings capture construction signatures without labels, distributed clustering separates normal from abnormal in 384-dimensional space, and outlier ranking surfaces the 1% of tiles that warrant human inspection.

This implementation processes 20,000 tiles (100-mile pipeline segment) in under 30 minutes on a GPU cluster, flagging 200 highest-anomaly locations for 24-hour field inspection. The architecture scales to continental networks: 3 million miles × 500m buffer × 50m tiles = 480M tiles processable in hours on modest Databricks infrastructure.

The economics are compelling: daily satellite monitoring at $0.03/mile/day ($10,950/year for 10,000 miles) vs monthly helicopter patrol at $5M-20M/year. The safety case is even stronger: detecting encroachment within 24 hours instead of 15-30 days means intervention before excavation starts, not after pipe rupture.

You can use Delta Live Tables for incremental ETL, Unity Catalog for image + embedding governance, MLlib for distributed ML, and Mosaic for geospatial visualization. Change catalog paths, add your corridor geometry, point to your imagery. The model is pre-trained, the infrastructure is scalable, and the alerts are actionable.

Williams paid $58 million for missing an encroachment by three days. What's your monthly patrol schedule costing you?
