...
def create_main_embedding_space_plot(plot: bool = False):
    ...
    if plot:
        ...
        ax.set_title(
            "ROW Encroachment Detection via DINOv2 Embeddings",
            fontsize=12,
            fontweight="bold",
            loc="left",
            pad=20,
    ...
    logger.info(
        f"  Encroachments: {n_encroachment} ({n_encroachment / len(labels) * 100:.1f}%)"
    )
...
def create_temporal_encroachment_trend(plot: bool = False):
    ...
    if plot:
        ...
        ax.set_title(
            "ROW Encroachment Trends Over Time",
            fontsize=13,
            fontweight="bold",
            loc="left",
            pad=20,
